...
function Header() {
  ...
  return (
    <header className="d-flex justify-content-between align-items-center px-2 mx-2 px-xl-5 mx-xl-5 flex-wrap ">
      <h2
        className="text-uppercase flex-item cursor-pointer"
        onClick={() => {
          navigate("/");
        }}
      >
      ...
      <div className="globalSearchContainer d-flex me-3 border border-primary  align-items-center flex-item bg-white">
        <label htmlFor="globalSearch" className="d-flex align-items-center">
          {" "}
          <span className=" mx-3 icon-search fs-4 text-muted"></span>
        </label>
        <input
          type="text"
          name="globalSearch"
          ...
          placeholder="search for club or national jersey"
        />
      </div>
      <span
        className="mx-3 justify-content-center align-items-center d-none d-md-flex flex-item"
        onClick={() => {
          navigate("wishlist");
        }}
      >
      ...
      <div
        className=" mx-3 d-flex justify-content-center align-items-center flex-item position-relative   py-4"
        onClick={() => {
          navigate("cart");
        }}
      >
        {cart.length > 0 && (
          <small
            className={`${badgeClass} header-cart-badge position-absolute bg-primary text-white  text-center rounded-circle`}
          >
            {cartItemNumber}
          </small>
        )}
      ...
      </div>
      {
        !userAuthData && (
          <button
            className="btn btn-dark rounded-0 mx-3 px-md-4 py-md-2 flex-item"
            onClick={() => {
              navigate("/redirect");
            }}
          >
      ...
      }
      {
        userAuthData && (
          <span
            className="icon-user fs-3 mx-3 flex-item "
            onClick={() => {
              navigate("/profile");
            }}
          ></span>
